...
REVOKE EXECUTE ON FUNCTION app_private.register_business(text, text, text, text, text, text) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION app_private.register_business(text, text, text, text, text, text) TO authenticated;
...
SET search_path = public
AS $$
  SELECT app_private.register_business(
    _business_name,
    _email,
    _phone,
    _country,
